...
export default function MriAnalyzerModal({ isOpen, onClose, onRefresh, patient, summary, scanData }: MriAnalyzerModalProps) {
  ...
  const startAnalysis = async () => {
    ...
    setPipelineLogs(["[INFO] Initiating 6-Layer Deep Scientific Pipeline..."]);
    
    try {
      const token = localStorage.getItem('hospital_token');
      
      const response = await fetch(`/api/v1/patients/${patient?.patient_uid}/analyze`, {
        method: 'GET',
        headers: {
      ...
      if (!response.ok) {
        throw new Error(`Pipeline handshake failed: ${response.statusText}`);
      }
