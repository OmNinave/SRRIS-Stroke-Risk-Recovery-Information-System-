  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.response?.status === 401) {
        setError("Invalid clinician ID or password. Use 'admin123' for recovery.");
      } else if (!err.response) {
        setError("Portal unreachable. Ensure backend service is running on port 8000.");
      } else {
        setError(detail || "Portal handshake failed. Verify departmental credentials.");
      }
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-screen flex flex-col lg:flex-row clinical-gradient relative overflow-hidden mesh-bg">
      ...
      <div className="flex-1 p-8 lg:p-16 flex flex-col justify-center relative z-10 max-w-4xl">
        <div className="hospital-brand flex items-center gap-3 mb-12">
          ...
          <div>
            <h2 className="text-slate-900 font-black leading-none text-2xl tracking-tighter">SRRIS <span className="text-blue-600">V3.5</span></h2>
            <p className="text-slate-600 text-[10px] uppercase tracking-[0.3em] font-bold mt-1">Scientific Intelligence System</p>
          </div>
        </div>
        ...
        <div className="mb-12">
          ...
          <p className="text-slate-600 text-lg max-w-2xl leading-relaxed">
            A dual-engine clinical framework combining <span className="text-blue-600 font-semibold">Stacked Ensembles</span> for risk stratification and <span className="text-blue-600 font-semibold">DeepSurv</span> for longitudinal recovery modeling.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
          <div className="meth-item stat-card">
            <div className="flex items-center gap-3 mb-3">
              ...
              <h3 className="text-slate-900 font-bold text-sm">Stacked Ensemble V3</h3>
            </div>
          ...
          <div className="meth-item stat-card">
            <div className="flex items-center gap-3 mb-3">
              <Workflow className="text-cyan-600" size={20} />
              <h3 className="text-slate-900 font-bold text-sm">Survival Trajectory</h3>
            </div>
          ...
          <div className="meth-item stat-card">
            <div className="flex items-center gap-3 mb-3">
              ...
              <h3 className="text-slate-900 font-bold text-sm">High-Fidelity Corpus</h3>
            </div>
          ...
          <div className="meth-item stat-card">
            <div className="flex items-center gap-3 mb-3">
              <Microchip className="text-emerald-600" size={20} />
              <h3 className="text-slate-900 font-bold text-sm">Explainable AI (XAI)</h3>
            </div>
        ...
        <div className="flex items-center gap-8 mt-4 border-t border-slate-200 pt-12">
          <div className="flex flex-col">
            <span className="text-2xl font-bold text-slate-900">99.4%</span>
            <span className="text-[10px] text-slate-600 uppercase tracking-widest font-black">Model Accuracy</span>
          </div>
          <div className="w-px h-10 bg-slate-200"></div>
          <div className="flex flex-col">
            <span className="text-2xl font-bold text-slate-900">500+</span>
            <span className="text-[10px] text-slate-600 uppercase tracking-widest font-black">Patient Profiles</span>
          </div>
          <div className="w-px h-10 bg-slate-200"></div>
          <div className="flex flex-col">
            <span className="text-2xl font-bold text-slate-900">V3.5</span>
            <span className="text-[10px] text-slate-600 uppercase tracking-widest font-black">Active Build</span>
          </div>
        </div>
      </div>
        ...
        <div className="login-card w-full max-w-[380px]">
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            <div className="mt-8 text-center">
              <button 
                type="button"
                onClick={() => alert("Developer Mode: Valid IDs are 'dr_smith', 'NEURO_DEPT', 'dr_jones', 'test'. All passwords have been reset to 'admin123'.")}
