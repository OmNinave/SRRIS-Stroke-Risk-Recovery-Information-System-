...
import { ArrowLeft, ChevronLeft, ChevronRight, FileText, Download, LayoutTemplate, FolderOpen, Folder, ScanEye, ShieldAlert, Activity, BrainCircuit, Beaker, Zap, Microscope, Camera, PenLine, Trash2 } from 'lucide-react';
...
export default function DocumentVaultPage() {
  ...
  const handleDeleteDocument = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.post(`/api/v1/patients/${uid}/documents/${activeDoc.id}/delete`, {
      ...
      // Refresh documents
      const res = await axios.get(`/api/v1/patients/${uid}/documents`, { headers: { Authorization: `Bearer ${token}` } });
      setDocs(res.data);
      if (res.data.length > 0) {
        const updatedGrouped = res.data.reduce((acc: any, doc: any) => {
          const cat = doc.category || "General Records";
          if (!acc[cat]) acc[cat] = [];
          acc[cat].push(doc);
          return acc;
        }, {});
        const updatedCats = Object.keys(updatedGrouped);
        if (updatedGrouped[activeCategory]?.length > 0) {
          setActiveIndex(0);
        } else if (updatedCats.length > 0) {
          setActiveCategory(updatedCats[0]);
          setActiveIndex(0);
        } else {
          setActiveCategory('');
          setActiveIndex(0);
      ...
      setDeleteError(err.response?.data?.detail || 'Failed to authenticate and delete document.');
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  useEffect(() => {
    const fetchDocs = async () => {
      try {
        ...
        setTimeout(() => {
          if (document.querySelector('.vault-anim')) {
  ...
  const handleStartScan = async () => {
    ...
    try {
      ...
      }, 5000); // Poll every 5s — Gemini retries take ~10s each, 3s was excessive
    ...
    <div className="min-h-screen bg-slate-900 text-white flex flex-col font-sans h-screen overflow-hidden">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      {activeDoc && (
                        <button 
                          ...
                          className="text-red-400 hover:text-red-300 p-2 bg-white/5 rounded-full transition-colors hover:scale-105 active:scale-95"
                      ...
                      <div className={`relative vault-anim transition-all duration-500 ${isDocScanning ? 'scale-[0.98] outline-[6px] outline-blue-600 outline rounded-sm shadow-[0_0_100px_rgba(37,99,235,0.6)]' : ''}`}>
                        ...
                        {isDocScanning && (
                         ...
                         <img 
                          ...
                          src={
                            enableAi && scanResult
                              ? (vizMode === 'heatmap' ? (scanResult.heatmap_image || scanResult.detection_image) : scanResult.detection_image)
                              // Use static /uploads/ path — browser <img> cannot send Bearer tokens to a protected API
                              : activeDoc.file_path
                                  ? `/uploads/${activeDoc.file_path.split(/[/\\]uploads[/\\]/i)[1]?.replace(/\\/g, '/')}`
                                  : `/api/v1/patients/${uid}/documents/${activeDoc.id}`
                          }
                          ...
                          onError={(e) => {
                            // Graceful fallback: show a placeholder if file not found
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 bg-slate-955/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-white/10 rounded-3xl p-8 max-w-md w-full shadow-2xl animate-in fade-in zoom-in duration-200 text-white">
            <h3 className="text-xl font-black text-white flex items-center gap-2 mb-2">
              <Trash2 className="text-red-500" size={24} /> Confirm Document Deletion
            </h3>
            <p className="text-sm text-slate-400 mb-6 leading-relaxed">
              This action will permanently purge <strong className="text-white">{activeDoc?.file_name}</strong> and all associated AI scan predictions, overlays, and features from the server.
            </p>
            ...
            <form onSubmit={handleDeleteDocument} className="space-y-4">
              ...
              <div>
                <label className="block text-[10px] font-black uppercase tracking-wider text-slate-500 mb-1.5">Attending Doctor Username</label>
                <input 
                  type="text" 
                  ...
                  required
                  className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-blue-500 focus:bg-black/60 transition-colors"
                />
              </div>

              <div>
                <label className="block text-[10px] font-black uppercase tracking-wider text-slate-500 mb-1.5">Verification Password</label>
                <input 
                  type="password" 
              ...
              <div className="flex gap-3 pt-4">
                <button 
                  ...
                  disabled={isDeleting}
                  className="flex-1 bg-white/5 hover:bg-white/10 border border-white/10 text-slate-300 py-3 rounded-xl text-sm font-bold transition-all"
                >
                  Cancel
                </button>
                <button 
                  type="submit" 
                  disabled={isDeleting}
                  className="flex-1 bg-red-600 hover:bg-red-700 text-white py-3 rounded-xl text-sm font-bold shadow-lg shadow-red-600/20 transition-all flex items-center justify-center gap-2"
                >
                  {isDeleting ? (
