...
import { 
  ...
  BrainCircuit, Droplet, Hospital, TrendingUp, Trash2
} from 'lucide-react';
...
export default function PatientDashboard() {
  ...
  const handleDeletePatient = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.post(`/api/v1/patients/${uid}/delete`, {
      ...
      setDeleteError(err.response?.data?.detail || 'Failed to authenticate and delete patient.');
    } finally {
      setIsDeleting(false);
    }
  };
        ...
        <div className="pt-8 pb-16 px-4 md:px-8">
          <nav className="max-w-7xl mx-auto w-full mb-8 flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              <Link href={`/patient/${uid}/recovery`} className="bg-white border border-indigo-200 px-4 py-2 rounded-xl text-sm font-bold flex items-center gap-2 text-indigo-600 hover:bg-indigo-50">
                <TrendingUp size={16} /> Recovery Intelligence
              </Link>
              ...
              <button 
                ...
                className="bg-red-50 hover:bg-red-100 text-red-600 border border-red-100 px-4 py-2 rounded-xl text-sm font-bold flex items-center gap-2 transition-all hover:scale-105 active:scale-95"
              >
                <Trash2 size={16} /> Expunge Patient
              </button>
            </div>
          </nav>
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-3xl p-8 max-w-md w-full shadow-2xl animate-in fade-in zoom-in duration-200">
            <h3 className="text-xl font-black text-slate-950 flex items-center gap-2 mb-2">
              <Trash2 className="text-red-500" size={24} /> Confirm Secure Deletion
            </h3>
            <p className="text-sm text-slate-500 mb-6 leading-relaxed">
              This action will permanently expunge <strong className="text-slate-800">{patient?.full_name}</strong> and all associated files, clinical events, scans, medications, and laboratory data from the registry and fileserver.
            </p>
            ...
            <form onSubmit={handleDeletePatient} className="space-y-4">
              ...
              <div>
                <label className="block text-[10px] font-black uppercase tracking-wider text-slate-400 mb-1.5">Attending Doctor Username</label>
                <input 
                  type="text" 
                  ...
                  required
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-800 focus:outline-none focus:border-blue-500 focus:bg-white transition-colors"
                />
              </div>

              <div>
                <label className="block text-[10px] font-black uppercase tracking-wider text-slate-400 mb-1.5">Verification Password</label>
                <input 
                  type="password" 
              ...
              <div className="flex gap-3 pt-4">
                <button 
                  ...
                  disabled={isDeleting}
                  className="flex-1 bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-700 py-3 rounded-xl text-sm font-bold transition-all animate-all"
                >
                  Cancel
                </button>
                <button 
