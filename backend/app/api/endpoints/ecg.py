...
def _run_ecg_in_thread(job_id: str, file_path: str, output_dir: str, patient_data: dict):
    ...
    try:
        ...
        result = predict_stroke_ecg(file_path, output_dir, patient_data=patient_data)
...
def scan_ecg_report(
    ...
    # Fetch patient metadata for personalizing the AI model
    ...
    patient_data = {
        ...
        'height': (patient.height_cm if patient and patient.height_cm is not None else 170.0),
        'weight': (patient.weight_kg if patient and patient.weight_kg is not None else 70.0)
    }
    ...
    thread = threading.Thread(
        ...
        args=(job_id, doc.file_path, output_dir, patient_data),
        daemon=True
    )
    thread.start()
