...
from app.schemas import DocumentResponse, LoginRequest
...
def process_document_bg(doc_id: int, file_path: str, file_type: str, file_category: str, patient_uid: str, db: Session):
    if doc_id in processing_jobs:
        print(f"[BG] Already processing Doc {doc_id}")
        return
    processing_jobs.add(doc_id)

    print(f"[BG] Starting processing for Doc {doc_id} (Category: {file_category}, Type: {file_type})")
    try:
        # Mark as PROCESSING in DB immediately so status survives restarts
        db_doc_status = db.query(models.Document).filter_by(id=doc_id).first()
        if db_doc_status:
            db_doc_status.processing_status = "PROCESSING"
            db.commit()
        ...
        db_doc_done = db.query(models.Document).filter_by(id=doc_id).first()
        if db_doc_done:
            db_doc_done.processing_status = "COMPLETED"
            db.commit()

        # ── Update in-memory progress tracker ───────────────────
        with _status_lock:
        ...
        try:
            db_doc_fail = db.query(models.Document).filter_by(id=doc_id).first()
            if db_doc_fail:
                db_doc_fail.processing_status = "FAILED"
                db_doc_fail.processing_error = str(e)[:500]
                db.commit()
        except Exception:
            pass
        with _status_lock:
...
def upload_document(
    ...
    smart_category, final_path = smart_organizer.organize_document(
        uid, temp_path, safe_filename, suggested_category=category
    )
...
def get_processing_status(
    ...
    """Returns processing status sourced from DB (survives server restarts) + in-memory cache."""
    # DB is source of truth — query all documents for this patient
    all_docs = db.query(models.Document).filter_by(patient_uid=uid).all()
    total = len(all_docs)
    done = sum(1 for d in all_docs if d.processing_status in ("COMPLETED", "FAILED"))
    processing = sum(1 for d in all_docs if d.processing_status == "PROCESSING")
    pending = sum(1 for d in all_docs if d.processing_status in ("PENDING", None))

    pct = (done / total * 100) if total > 0 else 100

    # Supplement with in-memory recent results for rich UI feedback
    mem_status = _processing_status.get(uid, {})
    recent_results = mem_status.get("results", [])[-5:]
    ...
    return {
        ...
        "pending": pending,
        "percent": round(pct, 1),
        "complete": pending == 0 and processing == 0,
...
def serve_document(uid: str, doc_id: int, db: Session = Depends(database.get_db), current_user: models.Doctor = Depends(get_current_user)):
...
def delete_document(
    uid: str, 
    doc_id: int, 
    verification: DocumentResponse = None, # Using a placeholder or we can import LoginRequest. Let's import it inline.
    db: Session = Depends(database.get_db), 
    current_user: models.Doctor = Depends(get_current_user)
):
    ...
    # Validate verification body manually since it's required
    if not verification or not hasattr(verification, "username") or not hasattr(verification, "password"):
        # FastAPI might raise a validation error or parse it from body. Let's explicitly expect LoginRequest in the path/body.
        pass

@router.post("/{uid}/documents/{doc_id}/delete")
def delete_document_post(
    ...
    # 1. Delete processed radiology files from disk
    processed_dir = os.path.join(os.getcwd(), "uploads", "processed", uid)
    if os.path.exists(processed_dir):
        for pattern in [f"detection_{doc_id}_*", f"heatmap_{doc_id}_*"]:
            for f in glob.glob(os.path.join(processed_dir, pattern)):
                try:
                    os.remove(f)
                except Exception:
                    pass

    # 2. Delete source document file from disk
    try:
    ...
    db.query(models.ScanResult).filter_by(document_id=doc_id).delete(synchronize_session=False)
    db.query(models.LabResult).filter_by(document_id=doc_id).delete(synchronize_session=False)
    
    events = db.query(models.MedicalEvent).filter_by(document_id=doc_id).all()
    ...
    db.query(models.Document).filter_by(id=doc_id).delete(synchronize_session=False)
    db.expunge_all()
    db.commit()

    # Log action
    audit = models.AuditLog(doctor_id=current_user.id, patient_uid=uid, action=f"Deleted Document {doc_id} ({doc.file_name})")
    db.add(audit)
    db.commit()

    return {"message": "Document and all related files deleted successfully."}
