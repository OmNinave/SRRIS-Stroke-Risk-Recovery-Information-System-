...
KNOWN_RANGES = {
    ...
    "tc/ hdl":        {"min": 3.0,  "max": 5.0,  "unit": "ratio"},
    "ldl / hdl":      {"min": 1.5,  "max": 3.5,  "unit": "ratio"},
    "hdl / ldl":      {"min": 0.4,  "max": 999,  "unit": "ratio"},
    "non-hdl":        {"min": 0,    "max": 160,  "unit": "mg/dL"},
}


def _determine_status(test_name: str, value_str: str, ref_low: Optional[str] = None, ref_high: Optional[str] = None) -> str:
    """Determine Normal/High/Low/Critical based on value vs reference range."""
    try:
        ...
        if lo is not None and hi is not None:
            if val > hi:
                return "critical_high" if val > hi * 1.5 else "high"
            if val < lo:
                return "critical_low" if val < lo * 0.7 else "low"
            return "normal"
...
def parse_lab_report(ocr_text: str) -> List[Dict[str, Any]]:
    ...
    # Pattern 3: Thyrocare/Complex Grid: "Test Name    Technology    Value    Unit    Ref Range"
    complex_grid_pattern = re.compile(
        r'^(.{3,40}?)\s{2,}[A-Z\s]{4,20}\s{2,}([\d.]+\s*[HhLl\*]?)\s{1,}([a-zA-Z/%µ]*)\s{0,}([\d.\-<> ]*)?$'
    )

    for line in lines:
        # Skip header rows, section titles
        if re.match(r'^(test|investigation|analyte|parameter|result|reference|biological|normal|description|unit|report|patient|doctor|date|lab|hospital|sr\.?\s*no\.?|s\.?\s*no\.?|technology|method)',
                    line, re.IGNORECASE):
            continue
        ...
        matched = False
        for pattern in [row_pattern, colon_pattern, complex_grid_pattern]:
...
def extract_lab_summary(results: List[Dict]) -> str:
    """Generate a human-readable summary of lab findings with directional flags."""
    if not results:
    ...
    if abnormal:
        lines.append(f"\n⚠️ CLINICAL ALERTS ({len(abnormal)}):")
        for r in abnormal:
            s = r["status"]
            if s == "critical_high": flag = "🚨 CRITICAL HIGH"
            elif s == "critical_low": flag = "🚨 CRITICAL LOW"
            elif s == "high": flag = "📈 HIGH"
            elif s == "low": flag = "📉 LOW"
            else: flag = "🟡 ABNORMAL"
    ...
    if normal:
        ...
        for r in normal[:8]:  # Show first 8 normal
            lines.append(f"  {r['test_name']}: {r['value']} {r['unit']}")
        if len(normal) > 8:
            lines.append(f"  ... and {len(normal) - 8} more normal values.")
