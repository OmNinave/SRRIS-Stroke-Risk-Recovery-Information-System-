...
class VisionService:
    def __init__(self):
        # Phase 2 fix: resolve path relative to this file — works on any machine
        _base = os.path.dirname(__file__)
        _candidates = [
            os.path.join(_base, "..", "..", "models", "srris_stroke_brain_v1.pkl"),
            os.path.join(_base, "..", "..", "models", "stroke_vision", "srris_stroke_brain_v1.pkl"),
        ]
        self.weights_path = next((p for p in _candidates if os.path.exists(p)), None)
        self.model = None
        if load_learner and self.weights_path:
            try:
            ...
            print("[VisionService] Weights for srris_stroke_brain_v1 not found, running in Simulation Mode.")
