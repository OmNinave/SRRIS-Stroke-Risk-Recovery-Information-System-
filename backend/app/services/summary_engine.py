...
def _ask_gemini(prompt: str) -> str:
    try:
        if not client:
            return "NLP Analysis Failed: API key not configured"
        ...
        return f"NLP Analysis Failed: {e}"

def detect_ischemic_risks(events, labs, db: Session) -> list:
    flags = []
    # Check Events via fast keyword matching instead of 20x Gemini calls
    for e in events:
        text = str(e.title + " " + str(e.description)).lower()
        if 'denies' in text or 'no history' in text: continue

        for risk in ISCHEMIC_FACTORS:
            if risk in text:
                source_name = "Manual Entry"
                if e.document_id:
                    doc = db.query(models.Document).filter_by(id=e.document_id).first()
                    source_name = doc.file_name if doc else "Deleted Document"
                flags.append({"message": f"Extracted Risk: {risk.title()}", "evidence": source_name})
                break # Avoid duplicate flags for same event
...
def detect_hemorrhagic_risks(events, db: Session) -> list:
    ...
    for e in events:
        ...
        for risk in HEMORRHAGIC_FACTORS:
            if risk in text:
                source_name = "Manual Entry"
                if e.document_id:
                    doc = db.query(models.Document).filter_by(id=e.document_id).first()
