...
def extract_clinical_terms(text: str) -> dict:
    """
    Semantic medical NER to extract key info from doctor notes using Gemini.
    ...
    try:
        ...
        print(f"[OCR NLP] Gemini init failed: {e}")
        return {"medications": [], "diagnoses": [], "vitals": {}, "instructions": [], "follow_up": None}

    prompt = f"""Extract the following clinical entities from the medical text below.
    ...
    {{
        ...
        "follow_up": "string describing follow up or null"
    }}
    ...
    try:
        ...
        print(f"[OCR NLP Extraction Error] {e}")
        return {
            "medications": [],
            "diagnoses": [],
            "vitals": {},
