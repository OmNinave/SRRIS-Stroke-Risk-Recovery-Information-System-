...
class Config:
    translate_eng_ru = {
        'down': 'Inferior',
        'front': 'Anterior',
        'front_down': 'Anterolateral',
        'front_septal': 'Anteroseptal',
        'normal': 'Normal',
        'septal': 'Septal',
        'side': 'Lateral'
    }
...
def predict_stroke_ecg(image_path: str, output_dir: str, patient_data: dict = None):
    """Main entry point for SRRIS ECG Analysis with sequential locking."""
    if patient_data is None:
        patient_data = {'age': 65.0, 'sex': 1, 'height': 170.0, 'weight': 70.0}
    ...
    # Use real patient metadata
    meta_df = pd.DataFrame({
        'age': [float(patient_data.get('age') or 65.0)],
        'sex': [1 if str(patient_data.get('gender') or 'male').lower() == 'male' else 0],
        'height': [float(patient_data.get('height') or 170.0)],
        'weight': [float(patient_data.get('weight') or 70.0)]
    })
    ...
    if max_target != 'норма' and max_p > 0.4:
        internal_name = Config.translate_ru_eng.get(max_target, max_target)
        clinical_name = Config.translate_eng_ru.get(internal_name, max_target)
    ...
    english_markers = {}
    for k, v in final_proba.items():
        internal_eng = Config.translate_ru_eng.get(k, k)
        clinical_eng = Config.translate_eng_ru.get(internal_eng, k)
        english_markers[clinical_eng] = f"{v:.2f}"
